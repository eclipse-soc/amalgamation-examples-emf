...
public class StringRAPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			// init values
			if (stringRA.getValue() != null && isAccessible(DroidViewsRepository.StringRA.Properties.value))
				basePart.setValue(EEFConverterUtil.convertToString(EcorePackage.eINSTANCE.getEString(), stringRA.getValue()));
			
			if (stringRA.getPackage() != null && isAccessible(DroidViewsRepository.StringRA.Properties.package_))
				basePart.setPackage_(EEFConverterUtil.convertToString(EcorePackage.eINSTANCE.getEString(), stringRA.getPackage()));
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (DroidViewsRepository.StringRA.Properties.value == event.getAffectedEditor()) {
			stringRA.setValue((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.eINSTANCE.getEString(), (String)event.getNewValue()));
		}
		if (DroidViewsRepository.StringRA.Properties.package_ == event.getAffectedEditor()) {
			stringRA.setPackage((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.eINSTANCE.getEString(), (String)event.getNewValue()));
		}
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {	
			...
			if (DroidPackage.eINSTANCE.getStringVA_Value().equals(msg.getFeature()) && basePart != null && isAccessible(DroidViewsRepository.StringRA.Properties.value)) {
			...
			if (DroidPackage.eINSTANCE.getResourceAccess_Package().equals(msg.getFeature()) && basePart != null && isAccessible(DroidViewsRepository.StringRA.Properties.package_)) {
				if (msg.getNewValue() != null) {
			...
			if (DroidPackage.eINSTANCE.getResourceAccess_ExternalResource().equals(msg.getFeature()) && basePart != null && isAccessible(DroidViewsRepository.StringRA.Properties.externalResource)) {
				if (msg.getNewValue() != null) {
			...
			try {
				if (DroidViewsRepository.StringRA.Properties.value == event.getAffectedEditor()) {
					...
					if (newValue instanceof String) {
						newValue = EcoreUtil.createFromString(DroidPackage.eINSTANCE.getStringVA_Value().getEAttributeType(), (String)newValue);
					}
					ret = Diagnostician.INSTANCE.validate(DroidPackage.eINSTANCE.getStringVA_Value().getEAttributeType(), newValue);
				}
				if (DroidViewsRepository.StringRA.Properties.package_ == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
